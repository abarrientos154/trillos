...
class QuotationController {
  ...
  async showAllChats ({ params, response, auth }) {
    ...
    let quotations = []
    if (rol === 2) {
      quotations = (await Quotation.query().where({ client_id: user._id, $or: [{ status: 0 }, { status: 1 }, { status: 2 }] }).with('data_supplier').with('data_request').with('lastMessage').fetch()).toJSON()
    } else if (rol === 3) {
      const id = new ObjectId(user._id)
      quotations = (await Quotation.query().where({ supplier_id: id, $or: [{ status: 0 }, { status: 1 }, { status: 2 }] }).with('data_client').with('data_request.categorianame').with('lastMessage').fetch()).toJSON()
    }
   ...
   * @param {View} ctx.view
   */
  async getQuotationById ({ params, response }) {
    const data = (await Quotation.query().find(params.id)).toJSON()
    response.send(data)
  }
   ...
   * @param {View} ctx.view
   */
  async editQuotation ({ params, request, response }) {
    let body = request.body
    body.supplier_id = new ObjectId(body.supplier_id)
    const editData = await Quotation.query().where('_id', params.id).update(body)
